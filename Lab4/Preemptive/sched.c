	...
	while (!load_done || !empty(&ready_queue)) {
		...
		if (proc == NULL) {
			...
			*/
			...
			else if (empty(&ready_queue) || (!empty(&ready_queue) && (&ready_queue)->head->data->priority >= proc->priority)) {
				while (empty(&ready_queue) || (!empty(&ready_queue) && (&ready_queue)->head->data->priority >= proc->priority)) {
					if (proc->burst_time - timeslot >= 0) {
						exec_time += timeslot;
...
void * loader(void * arg) {
	...
	while (!empty(&in_queue)) {
		...
		if (proc->arrival_time > timestamp && !empty(&in_queue)) {
			struct pcb_t * procTemp = proc;
			proc = de_queue(&in_queue);
			en_queue(&in_queue, procTemp);
		}
		int wastetime = proc->arrival_time - timestamp;
		if (wastetime < 0) wastetime = 0;
...
void load_task() {
	...
	int num_proc = 0;
	fscanf(ptr, "%d %d", &timeslot, &num_proc);
	int i;
	for (i = 0; i < num_proc; i++) {
		struct pcb_t * proc = (struct pcb_t *)malloc(sizeof(struct pcb_t));
		fscanf(ptr, "%d %d %d", &proc->arrival_time, &proc->burst_time, &proc->priority);
		proc->pid = i;
